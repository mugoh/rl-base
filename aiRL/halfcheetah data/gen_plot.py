...
def plot():
    ...
    default_hc = 'default_hc_run_250itrs.csv'

    iRL_pi_path = '3_with_softppo_comparison/airl_soft_ppo_.csv'
    std_pi_path = '3_with_softppo_comparison/used_expert_return_maxrew100.csv'

    paths = std_pi_path, iRL_pi_path,
    # paths = std_pi_path,

    titles = ['Expert policy', 'iRL policy']
    # titles = ['PPO expert policy']

    legends = [draw_plot_plt(path, titles[i], False)
               for i, path in enumerate(paths)]

    plt.suptitle('Comparison of iRL Policy with Expert Policy')
    plt.title('iRL trained on demos from this expert policy', size=8)
